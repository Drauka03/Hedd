...
lib.classes["DRUID"][1] = function() --Moonkin
	...
	lib.AddCleaveSpell("Starfall",{191034})
	lib.AddCleaveSpell("Lunar Strike",{194153})
	...
	lib.AddAura("Celestial Alignment",194223,"buff","player")
	lib.AddAura("Lunar Empowerment",164547,"buff","player")
	lib.AddAura("Moonfire",164812,"debuff","target")
	lib.AddAura("Moonkin Form",24858,"buff","player")
	...
	lib.AddAura("Warrior of Elune",202425,"buff","player")
	...
	table.insert(cfg.plistdps,"Moonfire_4.8")
	table.insert(cfg.plistdps,"Sunfire_3.6")
	table.insert(cfg.plistdps,"Stellar Flare_7.2")
	table.insert(cfg.plistdps,"Lunar Strike_filler")
	...
	cfg.case = {
		...
		["Lunar Strike_filler"] = function()
			if cfg.Power.now>=cfg.Power.max-12 then return nil end
			if lib.GetAuraStacks("Solar Empowerment")>=1 then return nil end
		...
		["Solar Wrath_filler"] = function()
			if cfg.Power.now>=cfg.Power.max-8 then return nil end
			if lib.GetAuraStacks("Lunar Empowerment")>=1 then return nil end
			if cfg.cleave_targets<2 then
				return lib.SimpleCDCheck("Solar Wrath")
			end
			return nil
	...
	["Stellar Flare"] = function()
		...
		if lib.GetAura({"Stellar Flare"})<=0 then
		return lib.SimpleCDCheck("Stellar Flare")
	end
	return nil
end,
		["Stellar Flare_7.2"] = function ()
			if cfg.Power.now>=cfg.Power.max-8 then return nil end
			if lib.SpellCasting("Stellar Flare") then return nil end
			-- return lib.SimpleCDCheck("Stellar Flare",lib.GetAura({"Stellar Flare"})-lib.GetAuraRefresh("Stellar Flare"))
			if lib.GetAura({"Stellar Flare"})<=7.2 then
				return lib.SimpleCDCheck("Stellar Flare")
			end
			return nil
		end,
		["Sunfire"] = function()
			if lib.GetAura({"Sunfire"})<=0 then
				return lib.SimpleCDCheck("Sunfire")
			end
			return nil
		end,
		["Sunfire_3.6"] = function ()
			if cfg.Power.now>=cfg.Power.max-3 then return nil end
			if lib.GetAura({"Sunfire"})<=3.6 then
				return lib.SimpleCDCheck("Sunfire")
			end
			return nil
...
lib.classes["DRUID"][3] = function() --Bear
	...
	cfg.talents = lib.ScanTalents()
	lib.ScanSpells()
	lib.RemoveSpell("Thrash")
	lib.RemoveSpell(106832)
	...
	table.insert(cfg.plistdps,"Incarnation_Thrash")
	...
	cfg.case = {
		["Bear Form"] = function()
			if not lib.GetAura({"Bear Form"}) then
				return lib.SimpleCDCheck("Bear Form")
			end
			return nil
		end,
